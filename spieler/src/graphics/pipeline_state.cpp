...
namespace spieler
{
    ...
    GraphicsPipelineState::GraphicsPipelineState(Device& device, const Config& config)
    {
        ...
        SPIELER_D3D12_ASSERT(device.GetD3D12Device()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&m_D3D12PipelineState)));
    }
    ...
    ComputePipelineState::ComputePipelineState(Device& device, const Config& config)
    {
        ...
        SPIELER_D3D12_ASSERT(device.GetD3D12Device()->CreateComputePipelineState(&computePipelineStateDesc, IID_PPV_ARGS(&m_D3D12PipelineState)));
    }
