...
namespace spieler
{
	...
	GraphicsCommandList::GraphicsCommandList(Device& device)
	{
        ...
        SPIELER_D3D12_ASSERT(device.GetD3D12Device()->CreateCommandAllocator(commandListType, IID_PPV_ARGS(&m_D3D12CommandAllocator)));

        SPIELER_D3D12_ASSERT(device.GetD3D12Device()->CreateCommandList(
			0,
			commandListType,
	...
	{
        SPIELER_D3D12_ASSERT(m_D3D12GraphicsCommandList->Close());
	}

	void GraphicsCommandList::Reset(const PipelineState* const pso)
	{
        // Reset CommandAllocator
        SPIELER_D3D12_ASSERT(m_D3D12CommandAllocator->Reset());
        
        // Reset GraphicsCommandList
		ID3D12PipelineState* d3d12PSO = pso ? pso->GetD3D12PipelineState() : nullptr;
		SPIELER_D3D12_ASSERT(m_D3D12GraphicsCommandList->Reset(m_D3D12CommandAllocator, d3d12PSO));
	}
    ...
    {
        SPIELER_ASSERT(resource.GetD3D12Resource());
    ...
    {
        SPIELER_ASSERT(buffer.GetD3D12Resource());
        SPIELER_ASSERT(size);
        SPIELER_ASSERT(data);
    ...
    void GraphicsCommandList::UploadToTexture(TextureResource& texture, const std::vector<SubResourceData>& subResources)
    {
        ...
        SPIELER_ASSERT(texture.GetD3D12Resource());
