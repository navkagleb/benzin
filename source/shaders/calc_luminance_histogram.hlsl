...
void CsMain(uint2 pixelPosition : SV_DispatchThreadID, uint binIndex : SV_GroupIndex)
{
    ...
    if (all(pixelPosition < g_FrameConsts.m_RenderResolution))
    {
...

    {
        ...
        const float maxBinValue = (g_FrameConsts.m_RenderResolution.x * g_FrameConsts.m_RenderResolution.y) / (g_ThreadCountX * g_ThreadCountY);
