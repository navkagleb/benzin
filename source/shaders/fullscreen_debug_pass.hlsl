...
float4 PS_Main(VS_FullScreenTriangleOutput input) : SV_Target
{
    ...
    packedGBuffer.Color0 = albedoAndRoughnessTexture.SampleLevel(g_PointClampSampler, input.UV, 0);
    packedGBuffer.Color1 = emissiveAndMetallicTexture.SampleLevel(g_PointClampSampler, input.UV, 0);
    packedGBuffer.Color2 = worldNormalTexture.SampleLevel(g_PointClampSampler, input.UV, 0);
    packedGBuffer.Color3 = float4(velocityBuffer.SampleLevel(g_PointClampSampler, input.UV, 0), 0.0f, 0.0f);
    ...
    switch (passConstants.OutputType)
    {
        ...
        {
            const float viewDepth = viewDepthBuffer.SampleLevel(g_PointClampSampler, input.UV, passConstants.ViewDepthMipIndex);
            return float4((viewDepth + passConstants.MinViewDepth) / passConstants.MaxViewDepth, 0.0, 0.0, 1.0);
        }
        case joint::DebugOutputType_CurrentShadowVisibility:
        {
            const float shadowVisibility = shadowVisibilityBuffer.SampleLevel(g_PointClampSampler, input.UV, passConstants.ViewDepthMipIndex);
