...
namespace sandbox
{
	...
	void GeometryPass::OnExecute(const entt::registry& registry, const benzin::BufferResource& entityBuffer)
	{
		...
		auto& gbufferResources = m_Resources[m_SwapChain.GetCurrentBackBufferIndex()];
...

		{
			...
			commandList.SetRootConstantBuffer(RootConstant::PassBufferIndex, gbufferResources.PassBuffer->GetConstantBufferView());
