...
namespace sandbox
{
    ...
    private:
        struct FrameContext
        {
            std::shared_ptr<benzin::BufferResource> PassBuffer;
            std::shared_ptr<benzin::BufferResource> EntityBuffer;
            std::shared_ptr<benzin::BufferResource> MaterialBuffer;
        };
    ...
    private:
        ...
        void DrawEntities(uint32_t entityBufferOffsetIndex, const benzin::PipelineState& pso, const std::span<const benzin::Entity*>& entities);
        void DrawFullscreenQuad(const benzin::Descriptor& srv);
    ...
    private:
        ...
        bool m_IsSobelFilterPassEnabled{ true };
        bool m_UseSobelFilterEdgeMap{ false };
        std::unique_ptr<SobelFilterPass> m_SobelFilterPass;

        std::unique_ptr<BlurPass> m_BlurPass;
        BlurPass::ExecuteArgs m_BlurPassExecuteArgs;
