...
namespace benzin
{
    ...
    using ADL2_ADAPTER_GRAPHIC_CORE_INFO_GET = int(*)(ADL_CONTEXT_HANDLE context, int iAdapterIndex, ADLGraphicCoreInfo* pGraphicCoreInfo);
    ...
    ADL2_ADAPTER_GRAPHIC_CORE_INFO_GET ADL2_Adapter_Graphic_Core_Info_Get = nullptr;
    ...
    {
        constexpr std::string_view vendorKey = "VEN_";
        constexpr std::string_view deviceKey = "DEV_";
        constexpr std::string_view subSysKey = "SUBSYS_";
        constexpr std::string_view revisionKey = "REV_";

        const auto GetValue = [](std::string_view udid, std::string_view key)
        {
    ...
    void AdlState::Initialize()
    {
        ...
        ADL2_Adapter_Graphic_Core_Info_Get = (decltype(ADL2_Adapter_Graphic_Core_Info_Get))::GetProcAddress(m_DllHandle, BenzinStringify(ADL2_Adapter_Graphic_Core_Info_Get));
    ...
    bool AdlState::IsAmdAdapter(int adlAdapterIndex)
    {
    ...
    uint32_t AdlWrapper::GetGpuCoreCount()
    {
        ADLGraphicCoreInfo aldGraphicCoreInfo = {};
        BenzinAdlEnsure(ADL2_Adapter_Graphic_Core_Info_Get(g_AdlState.m_Context, g_AdlState.m_AdlAdapterIndex, &aldGraphicCoreInfo));

        const uint32_t gpuCoreCount = aldGraphicCoreInfo.iNumWGPs * aldGraphicCoreInfo.iNumPEsPerWGP;
        return gpuCoreCount;
    }

    uint64_t AdlWrapper::GetUsedVramInBytes()
    {
        int vramUsageInMb = 0;
        BenzinAdlEnsure(ADL2_Adapter_VRAMUsage_Get(g_AdlState.m_Context, g_AdlState.m_AdlAdapterIndex, &vramUsageInMb));

        return vramUsageInMb * 1_mb;
    }

    uint64_t AdlWrapper::GetUsedDedicatedVramInBytes()
    {
        int vramUsageInMb = 0;
        BenzinAdlEnsure(ADL2_Adapter_DedicatedVRAMUsage_Get(g_AdlState.m_Context, g_AdlState.m_AdlAdapterIndex, &vramUsageInMb));
