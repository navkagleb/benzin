...
namespace sandbox
{
    ...
    CameraController::CameraController(Camera& camera)
        : m_Camera{ &camera }
    {
        m_Camera->AspectRatio = spieler::Application::GetInstance().GetWindow().GetAspectRatio();
    ...
    void CameraController::OnUpdate(float dt)
    {
        ...
        {   
            m_Camera->EyePosition = DirectX::XMVectorAdd(m_Camera->EyePosition, DirectX::XMVectorScale(DirectX::XMVector3Normalize(DirectX::XMVector3Cross(m_Camera->Front, m_Camera->Up)), m_CameraSpeed * dt));
            triggered = true;
        }

        if (input.IsKeyPressed(spieler::KeyCode::D))
        {
            m_Camera->EyePosition = DirectX::XMVectorSubtract(m_Camera->EyePosition, DirectX::XMVectorScale(DirectX::XMVector3Normalize(DirectX::XMVector3Cross(m_Camera->Front, m_Camera->Up)), m_CameraSpeed * dt));
            triggered = true;
        }
    ...
    void CameraController::OnImGuiRender(float dt)
    {
        ...
        ImGui::Begin("Camera");
        {
            ...
            if (ImGui::DragFloat3("Up", reinterpret_cast<float*>(&m_Camera->Up)))
            {
                UpdateView();
            }
    ...
    void CameraController::UpdateView()
    {
        ...
        m_Camera->View = DirectX::XMMatrixLookAtLH(m_Camera->EyePosition, DirectX::XMVectorAdd(m_Camera->EyePosition, m_Camera->Front), m_Camera->Up);
    }
